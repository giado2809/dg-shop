    ...
    <section class="checkout spad">
        <div class="container">
            <div class="checkout__form">
                <form action="{{ route('checkout.placeOrderOne', $cartItem->id) }}" method="POST">
                    @csrf
                    <div class="row">
                        <div class="col-lg-5">
                            <h6 class="coupon__code">
                                <span class="icon_tag_alt"></span> Có mã giảm? <a href="#">Nhập tại đây</a>
                            </h6>
                            ...
                            <div class="checkout__order">
                                ...
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        <tr>
                                            <td width="80">
                                                @php
                                                    $image = $cartItem->color && $cartItem->color->image
                                                        ? asset('storage/' . $cartItem->color->image)
                                                        : asset('storage/' . $cartItem->product->image);
                                                @endphp
                                                <img src="{{ $image }}" alt="{{ $cartItem->product->name }}">
                                            </td>
                                            <td>
                                                {{ $cartItem->product->name }}<br>
                                                @if ($cartItem->color_id)
                                                    <small class="text-muted">Màu: {{ optional($cartItem->color)->color ?? 'N/A' }}</small><br>
                                                @endif
                                                @if ($cartItem->size)
                                ...
                                <div class="form-group mt-3">
                                    ...
                                    <div class="input-group">
                                        <input type="text" name="voucher_code" id="voucher_code" class="form-control" placeholder="Nhập mã nếu có" value="{{ old('voucher_code') }}">
                                ...
                                <button type="submit" class="site-btn mt-3">PLACE ORDER</button>
                            </div>
                        </div>
                    </div>
                </form>
